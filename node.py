...
def lamport_on_receive(received_ts: int) -> int:
    """Update Lamport clock on receive: L = max(L, received_ts) + 1."""
    global LAMPORT
    with lock:
...
def replicate_to_peers(key: str, value: Any, ts: int, origin: str, retries: int = 2, timeout_s: float = 2.0) -> None:
    ...
    for peer in PEERS:
        ...
        if delay_s > 0:
            ...
            print(f"[{NODE_ID}] DELAYING replication to {peer} by {delay_s}s (Scenario A)")
            time.sleep(delay_s)
        ...
        for attempt in range(retries + 1):
            try:
                ...
                with request.urlopen(req, timeout=timeout_s) as resp:
                    _ = resp.read()
                print(f"[{NODE_ID}] Successfully replicated to {peer}")
                break
                ...
                if attempt == retries:
                    ...
                    print(f"[{NODE_ID}] Retrying {peer} in {backoff_time}s (attempt {attempt+1}/{retries})")
...
def main():
    ...
    # Для Scenario A: задержка от A к C
    if NODE_ID == "A":
        for peer in PEERS:
            if ":8002" in peer:  # Это нода C
                DELAY_RULES[(NODE_ID, peer)] = 5.0  # 5 секунд задержки
                print(f"[{NODE_ID}] Configured 5s delay to {peer} for Scenario A")
